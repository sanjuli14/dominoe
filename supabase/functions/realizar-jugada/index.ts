...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (parsedBody.isServerTrigger) {
      ...
      return new Response(
        JSON.stringify({ exito: true, mensaje: 'Bots procesados' }),
        {
          status: 200,
...
async function processJugada(
  ...
  if (!jugador) {
    throw new Error('No estás en la partida');
  }

  if (mano.turno_actual !== jugador.posicion) {
    throw new Error(
      `No es tu turno. Tu posición es ${jugador.posicion} y el turno actual es ${mano.turno_actual}`,
    );
  }
  ...
  if (restantes === 0) {
    const result = await finalizarManoPegada(
      supabaseAdmin,
      partida,
      mano,
      jugador,
  ...
  if (tranca) {
    const result = await finalizarManoTrancada(
      supabaseAdmin,
      partida,
      mano,
      jugador,
  ...
  while (playing) {
    ...
    if (botFichas && botFichas.length > 0) {
      for (const f of botFichas) {
        if (
          (mano.extremo_izquierdo === null && mano.extremo_derecho === null) ||
          f.valor_b === mano.extremo_izquierdo ||
          f.valor_a === mano.extremo_izquierdo
        ) {
          await processJugada(
            supabaseAdmin,
            currentJugador.user_id,
            partidaId,
            f.id,
            'izquierda',
          );
        ...
        if (
          f.valor_a === mano.extremo_derecho ||
          f.valor_b === mano.extremo_derecho
        ) {
          await processJugada(
            supabaseAdmin,
            currentJugador.user_id,
...
async function finalizarManoPegada(
  ...
  const jugadoresPerdedores = jugadores.filter(
    (j) => j.equipo === equipoPerdedor,
  );
  ...
  const puntosEquipoGanador =
    partida[`puntos_equipo_${ganador.equipo}`] + puntosTotales;
...
async function finalizarManoTrancada(
  ...
  // 4. Actualizar marcador
  const puntosEquipoGanador =
    partida[`puntos_equipo_${equipoGanador}`] + puntosGanados;
  const partidaFinalizada = puntosEquipoGanador >= META_PUNTOS;
